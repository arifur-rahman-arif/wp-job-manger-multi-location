jQuery(function ($) {
    if (!additionallocations) return;

    if (additionallocations.additionallocations) {
        var i = 0;
        // load existing locations
        $.each(additionallocations.additionallocations, function (k, v) {
            if (v) {
                // let clone = $('.fieldset-job_location').clone();
                $("#wpjmel_location .inside").append(
                    '<div><input type="text" style="width:100%" value="' +
                        additionallocations.additionallocations[k]["address"] +
                ...
                wpjmel_mb.lat = additionallocations.additionallocations[k]["lat"];
                wpjmel_mb.lng = additionallocations.additionallocations[k]["lng"];
